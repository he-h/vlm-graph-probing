...
def load_model(model_ckpt: str, device: str = "cuda:0"):
    ...
    if "llava" in lower:
        model_family = "llava"
        cls = LlavaNextForConditionalGeneration if "v1.6" in lower else LlavaForConditionalGeneration
        model = cls.from_pretrained(model_ckpt, torch_dtype=default_dtype).to(device).eval()
    elif "qwen" in lower:
        model_family = "qwen"
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            model_ckpt, torch_dtype=default_dtype,
        ).to(device).eval()
    elif "gemma" in lower:
        model_family = "gemma"
        ...
        raise ValueError(f"Unsupported model family for: {model_ckpt} (expected llava/qwen/gemma/internvl)")
...
__all__ = ["NeuronGraphExtractor", "GCNPredictor", "MLPPredictor", "build_corr_graph", "compute_corr_matrix"]
...
def compute_corr_matrix(hs: torch.Tensor) -> torch.Tensor:
    ...
    x = x / x.norm(dim=0).clamp_min(1e-8)

    C = (x.t() @ x).clamp(-1.0, 1.0)
    return C
...
def node_degrees(adj_matrix: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        degrees: [N] float32 tensor of node degrees (row sums of absolute values)
    """
    if adj_matrix.dim() != 2:
    ...
    degrees = adj_matrix.float().abs().sum(dim=1)
    return degrees
